...
- [ ] **Pre-payment print-area drift safeguard** — add a check inside the `checkout.session.completed` webhook handler that re-fetches Printful's live print-area for the customer's variant and verifies it matches our stored DB value within tolerance. If they don't match: do NOT send the order to Printful production; refund the payment and surface a clear error to the customer. This is the absolute last line of defense against the "editor showed X, hoodie arrived printed at Y" failure that becomes a refund + lost trust + unreturnable inventory. Lower-priority than the build-time validation that ships now (which catches the same class of bug earlier in the funnel) but worth having as belt-and-suspenders since incorrect-print refunds are the highest-cost failure mode.
