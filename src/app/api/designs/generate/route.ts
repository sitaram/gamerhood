...
import { bytesToBase64DataUrl, stripCheckerboardArtifacts } from "@/lib/print/normalize-upload";
    ...
    async start(controller) {
      ...
      try {
        ...
        for (const part of parts) {
          ...
          const generatedBytes = Buffer.from(base64, "base64");
          const checkerCleaned = await stripCheckerboardArtifacts(generatedBytes, mimeType);
          const imageUrl = bytesToBase64DataUrl(checkerCleaned.buffer, checkerCleaned.mimeOut);
          const imageBase64 = checkerCleaned.buffer.toString("base64");
          ...
          const imageCheck = await moderateImageBase64(imageBase64);
          if (!imageCheck.safe) {
          ...
          const transparency = await detectDesignTransparency(checkerCleaned.buffer);
          ...
          try {
            ...
            if (user) {
              ...
              if (profile) {
                ...
                if (designId) {
                  try {
                    const assets = await uploadDesignAssetDerivatives(designId, imageUrl);
                    const publicUrl = assets.printUrl;
